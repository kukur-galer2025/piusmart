...
<div x-data="{ 
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Kelola daftar pelanggan atau debitur Anda di sini.</p>
        </div>
    ...
    <div class="relative bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
        ...
        <div x-show="isSearching" 
            ...
            <div class="bg-white dark:bg-slate-800 px-5 py-2.5 rounded-full shadow-md border border-emerald-100 dark:border-slate-600 flex items-center gap-3 transform -translate-y-4">
                ...
                <span class="text-sm font-bold text-emerald-700 dark:text-emerald-400 tracking-wide animate-pulse">Mencari...</span>
            </div>
        </div>
    ...
    <div x-show="openDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak>
        <div @click.away="openDeleteModal = false" class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl max-w-sm w-full p-6 space-y-5 border border-transparent dark:border-slate-700">
            <div class="text-center">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">Hapus Pelanggan</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">Yakin menghapus <span class="font-bold text-gray-800 dark:text-gray-200" x-text="customerName"></span>?</p>
            </div>
            <div class="flex gap-3 mt-5">
                <button @click="openDeleteModal = false" class="flex-1 px-4 py-2.5 text-sm font-semibold text-gray-700 dark:text-slate-200 bg-gray-100 dark:bg-slate-700 rounded-xl hover:bg-gray-200 dark:hover:bg-slate-600 cursor-pointer">Batal</button>
                <form :action="deleteUrl" method="POST" class="flex-1">
                    @csrf @method('DELETE')
                    <button type="submit" class="w-full px-4 py-2.5 text-sm font-semibold text-white bg-rose-600 dark:bg-rose-500 rounded-xl hover:bg-rose-700 dark:hover:bg-rose-600 cursor-pointer">Hapus</button>
                </form>
            </div>
        </div>
    </div>
