...
<body class="bg-gray-50 dark:bg-slate-900 font-sans text-gray-900 dark:text-gray-100" 
    ...
    <div class="flex h-screen overflow-hidden relative">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700/50 transform transition-transform duration-300 md:translate-x-0 md:relative flex flex-col h-full shadow-2xl md:shadow-none"
            ...
            <div class="flex items-center justify-between h-16 px-5 bg-white dark:bg-slate-800 border-b border-gray-100 dark:border-slate-700/50 shrink-0">
                ...
                <button @click="sidebarOpen = false" class="md:hidden text-gray-400 hover:text-rose-500 p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-slate-700">
        ...
        <div class="flex flex-col flex-1 min-w-0 overflow-hidden relative">
            ...
            <header class="flex items-center justify-between h-16 px-4 md:px-6 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700/50 shrink-0 relative z-20">
                ...
                <div class="flex items-center gap-1 sm:gap-2 ml-auto relative">
                    ...
                    <div class="relative" x-data="{ notifOpen: false }">
                        ...
                        {{-- Dropdown Notif: full-width di mobile, popup di desktop --}}
                        <div x-show="notifOpen" @click.away="notifOpen = false" 
                             ...
                             class="fixed top-16 inset-x-0 sm:absolute sm:inset-auto sm:top-auto sm:-right-2 sm:mt-2 sm:w-96 bg-white dark:bg-slate-800 sm:rounded-2xl shadow-2xl border-t sm:border border-gray-100 dark:border-slate-700 z-[100] overflow-hidden origin-top-right" x-cloak>
                                ...
                                <div class="flex items-center gap-2">
                                    @if($unreadCountDB > 0)
                                        <form action="{{ route('notifications.mark-all-read') }}" method="POST" class="m-0 p-0">
                                            @csrf
                                            <button type="submit" class="text-[10px] font-bold text-emerald-600 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-slate-700 px-2 py-1 rounded-md border border-emerald-200/50 dark:border-emerald-500/30">
                                                {{ __('mark_all_read') }}
                                            </button>
                                        </form>
                                    @endif
                            ...
                            <div class="max-h-[50vh] sm:max-h-72 overflow-y-auto divide-y divide-gray-50 dark:divide-slate-700/50">
                                @if($unreadCountDB === 0)
                                ...
                                @else
                                    @foreach($dbNotifications as $notif)
                                        <a href="{{ route('notifications.index') }}" class="flex px-4 py-3.5 hover:bg-slate-50 dark:hover:bg-slate-700/50 gap-3">
            ...
            <footer class="shrink-0 px-4 py-3 sm:py-4 bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-slate-700/50 text-center flex flex-col items-center justify-center relative z-20">
                ...
                <button @click="aboutOpen = true" class="mt-1.5 text-[10px] sm:text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:underline focus:outline-none uppercase tracking-wider cursor-pointer">
                    {{ __('about_us') }}
                </button>
            </footer>
        </div>
    ...
    {{-- About Modal --}}
    <div x-show="aboutOpen" class="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/70 backdrop-blur-sm" x-cloak>
        <div @click.away="aboutOpen = false" x-show="aboutOpen" 
             x-transition:enter="transition ease-out duration-200"
             ...
             class="bg-white dark:bg-slate-800 rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-md overflow-hidden border-t border-emerald-100 dark:border-slate-700 sm:border">
            ...
            <div class="p-4 sm:p-5 bg-slate-50 dark:bg-slate-900 space-y-3">
                ...
                <div class="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700/50">
                    <div class="w-11 h-11 sm:w-13 sm:h-13 rounded-full overflow-hidden shrink-0 ring-2 ring-emerald-100 dark:ring-slate-700">
