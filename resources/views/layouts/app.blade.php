...
<body class="bg-gray-50 dark:bg-slate-900 font-sans text-gray-900 dark:text-gray-100 transition-colors duration-300" 
      x-data="{ 
          sidebarOpen: false, 
    ...
    <div class="flex h-screen overflow-hidden relative">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700/50 transform transition-transform duration-300 md:translate-x-0 md:relative flex flex-col h-full shadow-2xl md:shadow-none"
            ...
            <div class="flex items-center justify-between h-16 px-6 bg-white dark:bg-slate-800 border-b border-gray-100 dark:border-slate-700/50 shrink-0">
                <a href="{{ route('dashboard') }}" class="flex items-center hover:opacity-80">
                    <img src="{{ asset('images/logo.png') }}" alt="Piusmart Logo" class="h-10 w-auto object-contain dark:drop-shadow-[0_0_8px_rgba(255,255,255,0.7)]">
                </a>
                <button @click="sidebarOpen = false" class="md:hidden text-gray-400 hover:text-rose-500">
        ...
        <div class="flex flex-col flex-1 min-w-0 overflow-hidden relative">
            <header class="flex items-center justify-between h-16 px-4 md:px-6 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700/50 shrink-0 relative z-20 transition-colors duration-300">
                ...
                <div class="flex items-center gap-1.5 sm:gap-3 ml-auto relative">
                    <button @click="darkMode = !darkMode" class="p-1.5 sm:p-2 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-slate-700 dark:hover:text-emerald-400 rounded-full transition-colors duration-300">
                    ...
                    <div class="relative" x-data="{ notifOpen: false }">
                        <button @click="notifOpen = !notifOpen" class="relative p-1.5 sm:p-2 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-slate-700 dark:hover:text-emerald-400 rounded-full transition-colors duration-300">
                        ...
                        <div x-show="notifOpen" @click.away="notifOpen = false" 
                             ...
                             class="absolute -right-12 sm:-right-4 mt-3 w-80 sm:w-96 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-700 z-[100] overflow-hidden origin-top-right" x-cloak>
                                        ...
                                        <a href="{{ route('notifications.index') }}" class="flex px-5 py-3.5 hover:bg-slate-50 dark:hover:bg-slate-700/50 gap-3 block">
                                            <div class="flex-1">
                                                <p class="text-sm font-medium text-gray-800 dark:text-gray-200">
                                                    Tagihan <span class="font-bold">{{ $notif->data['customer_name'] ?? '' }}</span> butuh perhatian.
                                                </p>
                    ...
                    <div class="relative pl-1 sm:pl-2 border-l border-gray-200 dark:border-slate-700" x-data="{ profileOpen: false }">
                        ...
                        <div x-show="profileOpen" @click.away="profileOpen = false" x-transition class="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-100 dark:border-slate-700 py-1.5 z-50" x-cloak>
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="w-full text-left flex items-center px-4 py-2.5 text-sm font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-slate-700/50">
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto bg-slate-50/50 dark:bg-slate-900/50 relative z-10 flex flex-col transition-colors duration-300">
                <div class="p-4 md:p-8 flex-1">
                    @yield('content')
                </div>

                <footer class="mt-auto px-6 py-5 bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-slate-700/50 text-center flex flex-col items-center justify-center transition-colors duration-300">
                    <p class="text-sm font-semibold text-slate-700 dark:text-slate-300">
                        &copy; {{ date('Y') }} Piusmart Executive. All rights reserved.
                    </p>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        Sistem Manajemen Pencatatan Piutang Usaha Terpadu.
                    </p>
                    <button @click="aboutOpen = true" class="mt-2 text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:underline focus:outline-none uppercase tracking-wider">
                        TENTANG KAMI
                    </button>
                </footer>
            </main>
    ...
    <div x-show="aboutOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-0 bg-slate-900/70 backdrop-blur-sm" x-cloak>
        <div @click.away="aboutOpen = false" x-show="aboutOpen" x-transition.scale class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-md w-full overflow-hidden border border-emerald-100 dark:border-slate-700">
            <div class="relative bg-emerald-600 px-6 py-8 text-center text-white">
                <button @click="aboutOpen = false" class="absolute top-4 right-4 text-white/70 hover:text-white bg-black/10 hover:bg-black/20 p-1.5 rounded-full">
                ...
                <h3 class="text-xl font-extrabold tracking-tight">Tim Pengembang</h3>
            ...
            <div class="px-6 py-4 bg-white dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700">
                <button @click="aboutOpen = false" class="w-full py-2.5 text-sm font-bold text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-700 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-600">Tutup Panel</button>
            </div>
        </div>
    </div>
</body>
