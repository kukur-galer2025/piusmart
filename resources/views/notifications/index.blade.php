...
<div class="max-w-5xl mx-auto space-y-6" 
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-slate-800 p-5 md:p-6 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm transition-colors duration-300">
        <div class="flex items-center gap-4">
            <div class="p-3 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-xl hidden sm:block transition-colors">
            ...
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white transition-colors">{{ __('notification') }}</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 transition-colors">Pusat informasi, pemantauan, dan riwayat peringatan tagihan piutang.</p>
            </div>
        </div>
        
        @if(Auth::user()->unreadNotifications->count() > 0)
            <button @click="showConfirmModal = true" class="inline-flex items-center justify-center px-5 py-2.5 text-sm font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800/50 rounded-xl hover:bg-emerald-100 dark:hover:bg-emerald-800 hover:scale-[1.02] active:scale-95 transition-all cursor-pointer shadow-sm w-full sm:w-auto">
    ...
    <div class="border-b border-gray-200 dark:border-slate-700 transition-colors duration-300">
        <nav class="flex overflow-x-auto no-scrollbar space-x-8" aria-label="Tabs">
            <button @click="activeTab = 'belum_dibaca'"
                    ...
                    class="flex items-center whitespace-nowrap border-b-2 py-4 px-1 text-sm font-bold transition-colors cursor-pointer outline-none">
                Belum Dibaca
                <span :class="activeTab === 'belum_dibaca' ? 'bg-rose-100 text-rose-600 dark:bg-rose-900/40 dark:text-rose-400' : 'bg-gray-100 text-gray-600 dark:bg-slate-800 dark:text-slate-400'" 
                      class="ml-2 rounded-full py-0.5 px-2.5 text-xs font-bold transition-colors border border-transparent dark:border-slate-700">
                    {{ $unreadNotifs->count() }}
                </span>
            </button>
    ...
    <div x-show="activeTab === 'belum_dibaca'"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden transition-colors duration-300">
            <ul class="divide-y divide-gray-100 dark:divide-slate-700/50">
                @forelse($unreadNotifs as $notif)
                    ...
                    @endphp
                    <li class="p-4 sm:p-5 flex gap-4 bg-slate-50/60 dark:bg-slate-800/50 hover:bg-slate-100/80 dark:hover:bg-slate-700/50 transition-colors group">
                        <div class="shrink-0 mt-1">
                            @if($notif->data['type'] === 'overdue')
                                <div class="h-10 w-10 rounded-full bg-rose-100 dark:bg-rose-900/50 flex items-center justify-center ring-4 ring-white dark:ring-slate-800 shadow-sm transition-colors">
                            ...
                            @else
                                <div class="h-10 w-10 rounded-full bg-amber-100 dark:bg-amber-900/50 flex items-center justify-center ring-4 ring-white dark:ring-slate-800 shadow-sm transition-colors">
                        ...
                        <div class="flex-1 flex flex-col xl:flex-row justify-between items-start gap-3 xl:gap-4">
                            <div class="space-y-1">
                                <p class="text-sm font-bold text-gray-900 dark:text-gray-100 leading-snug transition-colors">
                                ...
                                <p class="text-xs text-gray-500 dark:text-gray-400 transition-colors">Tagihan: <span class="font-bold text-gray-700 dark:text-gray-300">Rp {{ number_format($notif->data['amount'] ?? 0, 0, ',', '.') }}</span></p>
                                <p class="text-[11px] font-semibold text-rose-500 dark:text-rose-400 flex items-center gap-1 pt-1 transition-colors">
                ...
                @empty
                    <li class="p-10 flex flex-col items-center justify-center text-center bg-white dark:bg-slate-800 rounded-2xl transition-colors">
                        <div class="h-12 w-12 rounded-full bg-emerald-50 dark:bg-emerald-900/30 flex items-center justify-center mb-3 text-emerald-500 dark:text-emerald-400 transition-colors">
                        ...
                        <p class="text-sm font-bold text-gray-500 dark:text-gray-400 transition-colors">Hebat! Semua notifikasi sudah Anda baca.</p>
                    </li>
                @endforelse
            </ul>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden transition-colors duration-300">
            <ul class="divide-y divide-gray-100 dark:divide-slate-700/50">
                @forelse($readNotifs as $notif)
                    ...
                    @endphp
                    <li class="p-4 sm:p-5 flex gap-4 bg-white dark:bg-slate-800 opacity-60 hover:opacity-100 transition-opacity">
                        <div class="shrink-0 mt-1">
                            <div class="h-10 w-10 rounded-full bg-gray-100 dark:bg-slate-700 flex items-center justify-center ring-4 ring-white dark:ring-slate-800 transition-colors">
                        ...
                        <div class="flex-1 space-y-1">
                            <p class="text-sm font-medium text-gray-600 dark:text-gray-300 leading-snug transition-colors">
                                {{ $notif->data['type'] === 'overdue' ? __('notif_overdue', ['name' => $notif->data['customer_name'] ?? '', 'days' => $days]) : __('notif_due_soon', ['name' => $notif->data['customer_name'] ?? '', 'days' => $days]) }}
                            </p>
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-400 dark:text-slate-500 transition-colors">
                ...
                @empty
                    <li class="p-10 flex flex-col items-center justify-center text-center bg-white dark:bg-slate-800 rounded-2xl transition-colors">
                        <div class="h-12 w-12 rounded-full bg-gray-50 dark:bg-slate-700/50 flex items-center justify-center mb-3 text-gray-300 dark:text-slate-500 transition-colors">
                        ...
                        <p class="text-sm font-medium text-gray-400 dark:text-slate-500 transition-colors">Belum ada riwayat notifikasi yang tersimpan.</p>
                    </li>
                @endforelse
            </ul>
        </div>
    ...
    <div x-show="showConfirmModal" 
        ...
        <div @click.away="showConfirmModal = false" 
             ...
             class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-sm w-full p-6 text-center overflow-hidden transition-colors duration-300">
            
            <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-indigo-50 dark:bg-indigo-900/30 mb-4 transition-colors">
            ...
            <h3 class="text-xl font-bold text-gray-900 dark:text-white transition-colors">Tandai Semua?</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-2 transition-colors">Apakah Anda yakin ingin menandai seluruh {{ $unreadNotifs->count() }} notifikasi menjadi "Sudah Dibaca"?</p>
            
            <div class="flex gap-3 mt-6">
                <button @click="showConfirmModal = false" class="flex-1 px-4 py-3 text-sm font-semibold text-gray-700 dark:text-slate-200 bg-gray-100 dark:bg-slate-700 rounded-xl hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors cursor-pointer">Batal</button>
                <form action="{{ route('notifications.mark-all-read') }}" method="POST" class="flex-1 m-0">
                    @csrf
                    <button type="submit" class="w-full px-4 py-3 text-sm font-bold text-white bg-emerald-600 dark:bg-emerald-500 rounded-xl hover:bg-emerald-700 dark:hover:bg-emerald-600 shadow-md shadow-emerald-600/20 transition-all cursor-pointer">Ya, Tandai</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div x-show="showSuccessModal" 
        ...
        <div @click.away="showSuccessModal = false" 
             ...
             x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-90 translate-y-8" x-transition:enter-end="opacity-100 scale-100 translate-y-0"
             x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-90 translate-y-8"
             class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-sm w-full p-8 text-center relative overflow-hidden border border-emerald-100 dark:border-slate-700 transition-colors duration-300">
            
            <div class="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-emerald-400 to-teal-500"></div>
            
            <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-emerald-100 dark:bg-emerald-900/30 mb-6 ring-4 ring-emerald-50 dark:ring-slate-800 transition-colors">
            ...
            <h3 class="text-xl font-extrabold text-slate-900 dark:text-white transition-colors">Berhasil!</h3>
            <p class="text-sm text-slate-500 dark:text-gray-400 mt-2.5 font-medium leading-relaxed transition-colors" x-text="successMessage"></p>
            
            <button @click="showSuccessModal = false" class="mt-8 w-full px-4 py-3 text-sm font-bold text-white bg-slate-900 dark:bg-slate-700 rounded-xl hover:bg-slate-800 dark:hover:bg-slate-600 shadow-md shadow-slate-900/20 transition-all cursor-pointer">{{ __('close') }}</button>
        </div>
    </div>
