...
<div x-data="{ 
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900 dark:text-white">{{ __('receivables_data') }}</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('manage_receivables_subtitle') }}</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full lg:w-auto">
            ...
            <a href="{{ route('receivables.statistics') }}" 
               title="{{ __('statistics') }}"
               class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2.5 text-sm font-semibold text-white bg-indigo-600 dark:bg-indigo-500 rounded-lg hover:bg-indigo-700 dark:hover:bg-indigo-600 shadow-sm cursor-pointer">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4 mr-2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" /></svg>
                📊 {{ __('statistics') }}
            </a>
    ...
    <div class="relative bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
        ...
        <div x-show="isSearching" 
            ...
            <div class="bg-white dark:bg-slate-800 px-5 py-2.5 rounded-full shadow-md border border-emerald-100 dark:border-slate-600 flex items-center gap-3 transform -translate-y-4">
                ...
                <span class="text-sm font-bold text-emerald-700 dark:text-emerald-400 tracking-wide animate-pulse">{{ __('loading_data') }}</span>
            </div>
        </div>
...
<script>
    ...
    function confirmPaid(id, customerName, amount) {
        let htmlContent = `{!! __('confirm_paid_text', ['name' => 'NAME_PLACEHOLDER', 'amount' => 'AMOUNT_PLACEHOLDER']) !!}`
            .replace('NAME_PLACEHOLDER', `<strong>${customerName}</strong>`)
            .replace('AMOUNT_PLACEHOLDER', `<span class="text-emerald-600 dark:text-emerald-400 font-extrabold">${amount}</span>`);

        Swal.fire({
            title: '{{ __('confirm_paid_title') }}',
            html: htmlContent,
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#059669',
            cancelButtonColor: '#9ca3af',
            confirmButtonText: '{{ __('yes_paid') }}',
    ...
    function confirmUnpaid(id, customerName, amount) {
        let htmlContent = `{!! __('confirm_unpaid_text', ['name' => 'NAME_PLACEHOLDER', 'amount' => 'AMOUNT_PLACEHOLDER']) !!}`
            .replace('NAME_PLACEHOLDER', `<strong>${customerName}</strong>`)
            .replace('AMOUNT_PLACEHOLDER', `<span class="text-rose-600 dark:text-rose-400 font-extrabold">${amount}</span>`);

        Swal.fire({
            title: '{{ __('confirm_unpaid_title') }}',
            html: htmlContent,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#e11d48',
            cancelButtonColor: '#9ca3af',
            confirmButtonText: '{{ __('yes_cancel') }}',
    ...
    function confirmDelete(id, customerName) {
        let htmlContent = `{!! __('confirm_delete_receivable_text', ['name' => 'NAME_PLACEHOLDER']) !!}`
            .replace('NAME_PLACEHOLDER', `<strong>${customerName}</strong>`);

        Swal.fire({
            title: '{{ __('delete_receivable_title') }}',
            html: htmlContent,
            icon: 'error',
            showCancelButton: true,
            confirmButtonColor: '#e11d48',
